...
public class Argumentation_sponges {
...
public static void main(String[] arg){
		...
		try {
			...
			int CB1 = TrainingSetUtils.DEMOSPONGIAE_120_DATASET;
			int CB2 = TrainingSetUtils.DEMOSPONGIAE_120_DATASET;
			...
			System.out.println(training_set_a1.cases.size());
			training_set_a1.cases = TrainingSetUtils.splitTrainingSet(training_set_a1.cases, 3, training_set_a1.description_path, training_set_a1.solution_path, dm_1, 0, 0).get(0);
			System.out.println(training_set_a1.cases.size());
			training_set_a2.cases = TrainingSetUtils.splitTrainingSet(training_set_a2.cases, 3, training_set_a2.description_path, training_set_a2.solution_path, dm_2, 0, 0).get(1);
			training_set_test.cases = TrainingSetUtils.splitTrainingSet(training_set_test.cases, 3, training_set_test.description_path, training_set_test.solution_path, dm_T, 0, 0).get(2);
			
			Agent_v1.DEBUG = 1;
			Concept.DEBUG = true;
			
			//FeatureTerm e = training_set_test.cases.get(0).featureValue(training_set_test.description_path.features.get(0));
			Token t = new Token();
			...
			/*for(FeatureTerm e : training_set_test.cases){
				examples.add(e.featureValue(training_set_test.description_path.features.get(0)));
			}*/
			
			LinkedList<FeatureTerm> needed = ((LinkedList<FeatureTerm>)training_set_test.differentSolutions());
			//can I seeneeded.addAll(((LinkedList<FeatureTerm>)training_set_test.differentSolutions()));
			Set<FeatureTerm> possible_examples = SetCast.cast(training_set_test.cases);
			
			while(!needed.isEmpty()){
				for(FeatureTerm ft : possible_examples){
					//System.out.println(ft.featureValue(training_set_test.solution_path.features.get(0)).toStringNOOS(dm_T));
					FeatureTerm si = needed.removeFirst();
					FeatureTerm sc = ft.featureValue(training_set_test.solution_path.features.get(0));
					sc = sc.featureValue("order");
					//System.out.println("tested "+sc.toStringNOOS(dm_T)+" \n wanted "+si.toStringNOOS(dm_T));
					if(sc.equals(si)){
			...
			while(agent_1.is_satisfied() && agent_2.is_satisfied() && !t.is_owned(agent_1) && !t.is_owned(agent_2)){
			}
